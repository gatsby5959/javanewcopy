...
public class DatabaseConnection {
	...
	private String jdbcUrl = "jdbc:mariadb://aj2002.cafe24.com:3306/aj2002";
	...
	private DatabaseConnection() {
		...
		try {
			Class.forName(jdbcDriver); //드라이버를 로드하기 위해 사용되는 메서드
			conn = DriverManager.getConnection(jdbcUrl,"aj2002","k2m16ak47"); //url 연결 메서드  url user password는 한세트
		} catch (ClassNotFoundException e) {
			System.out.println("드라이버를 찾을 수 없습니다.");
			e.printStackTrace();
		} catch (SQLException e) {
